...
namespace UpgradeHelpers.VB6.Gui
{
    ...
    public class ControlArrayHelper
    {
        ...
        public static void LoadControl(Control controlContainer, String controlName, int index)
        {
            ...
            FieldInfo fInfo = controlContainer.GetType().GetField(controlName);
            if ((fInfo != null) && (fInfo.FieldType.IsArray))
            {
                Array controlArray = (Array)fInfo.GetValue(controlContainer);
                ...
                if (baseControl != null)
                {
                    ...
                    fInfo.SetValue(controlContainer, controlArray);
                }
            }
        }
        ...
        public static void UnloadControl(Control controlContainer, String controlName, int index)
        {
            ...
            if ((fInfo != null) && (fInfo.FieldType.IsArray))
            {
                ...
                if (baseObject != null)
                {
                    ...
                    {
                        ((Control)baseObject).Parent.Controls.Remove((Control)objectToUnload);
                        controlArray.SetValue(null, index);
                        fInfo.SetValue(controlContainer, controlArray);
                    }
                    else if (baseObject is ToolStripItem)
                    {
                        ((ToolStripItem)baseObject).Owner.Items.Remove((ToolStripItem)objectToUnload);
                        controlArray.SetValue(null, index);
                        fInfo.SetValue(controlContainer, controlArray);
                    }
                }
        ...
        private static void CleanEventHandlers(object controlToUnload, object baseObject)
        {
            ...
            {
                Delegate[] eventDelegates = ContainerHelper.GetEventSubscribers(baseObject, eInfo.Name);
        ...
        private static Object CloneObject(Object baseControl)
        {
            ...
            if (baseControl != null)
            {
                if (baseControl is ICloneable)
                {
                    result = ((ICloneable)baseControl).Clone();
                }
                else
                {
                    ...
                    foreach (PropertyInfo pInfo in baseControl.GetType().GetProperties())
                    {
                        ...
                        else if (listOfProperties.ContainsKey(pInfo.Name))
                        {
                            try
                            {
                ...
                {
                    Delegate[] eventDelegates = ContainerHelper.GetEventSubscribers(baseControl, eInfo.Name);
                    //The event in the new control will be bound to the same delegates of the base control
                    if (eventDelegates != null)
                    {
                        foreach (Delegate del in eventDelegates)
