...
namespace UpgradeHelpers.VB6.VB
{
#pragma warning  disable 1591
    ...
    public class PrinterHelper
    {
        ...
        private int GetPixelsPerInchXFromDevice()
        {
            ...
            int caps = GetDeviceCaps(printerHdc, LOGPIXELSX);
        ...
        {
            IntPtr printerHdc = PrinterGraphics.GetHdc();
            int caps = GetDeviceCaps(printerHdc, LOGPIXELSY);
            PrinterGraphics.ReleaseHdc(printerHdc);
            return caps;
        }
        ...
        public int PaperBin
        {
            get { throw new Exception("Method or Property not implemented yet!"); }
            set { throw new Exception("Method or Property not implemented yet!"); }
        }
        ...
        public int FontCount
        {
            get
            {
                ...
                FontFamily[] families = FontFamily.GetFamilies(PrinterGraphics);
                        ...
                        for (int i = 0; i < _bufferPage.Images.Count; i++)
                        {
                            if (!_bufferPage.Images[i].clippingInfo.Equals(RectangleF.Empty))
                            {
                                ...
                                e.Graphics.DrawImage(_bufferPage.Images[i].Picture,
                                       srcRect,
                                       _bufferPage.Images[i].clippingInfo, GraphicsUnit.Pixel
                                       );
                            }
                            else
                            {
        ...
        private void SetCurrentHdcPenSettings()
        {
            ...
            if (currentGraphicsHdc != 0)
            {
                ...
                this.currentPen = CreatePen(this._drawStyle, newWidth, (uint)ColorTranslator.ToWin32(_currentColor));
                SelectObject((IntPtr)currentGraphicsHdc, this.currentPen);
            }
        }
        ...
        private void SetFontToHdc(float pointSize)
        {
            if (currentGraphicsHdc != 0)
            {
                ...
                int capSize = GetDeviceCaps(new IntPtr(currentGraphicsHdc), 90 /*LOGPIXELSY*/);
                float size = (capSize * pointSize) / 72f;
                Font tmpFont = new Font(_font.FontFamily, size, _font.Style, GraphicsUnit.Pixel);
                SelectObject(new IntPtr(currentGraphicsHdc), tmpFont.ToHfont());
            }
        }
 ...
 */
        ...
        [DllImport("gdi32.dll")]
        static extern int GetMapMode(int hdc);
        ...
        [DllImport("gdi32.dll")]
        static extern uint SetBkMode(IntPtr hdc, int crColor);
        [DllImport("gdi32.dll")]
